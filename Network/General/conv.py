    ...
    def __init__(self, args):
        ...
        if len(self.hs) == 0:
            if self.use_layer_norm: # activation final handles activativation
                layers = [nn.GroupNorm(1, self.hs[0]), nn.Conv1d(self.object_dim, self.output_dim, 1)]
            else:
                layers = [nn.Conv1d(self.object_dim, self.output_dim, 1)]
        else:
            if len(self.hs) == 1:
                if self.use_layer_norm:
                    layers = [nn.Conv1d(self.object_dim, self.hs[0], 1), get_inplace_acti(args.activation), nn.GroupNorm(1, self.hs[0])]
                else:
                    layers = [nn.Conv1d(self.object_dim, self.hs[0], 1), get_inplace_acti(args.activation)]
            else:
                if self.use_layer_norm:
                    layers = ([nn.Conv1d(self.object_dim, self.hs[0], 1), get_inplace_acti(args.activation),nn.GroupNorm(1, self.hs[0])] + 
                    sum([[nn.Conv1d(self.hs[i-1], self.hs[i], 1), get_inplace_acti(args.activation), nn.GroupNorm(1, self.hs[i])] for i in range(1, len(self.hs) - 1)], list())
                        + [nn.Conv1d(self.hs[-2], self.hs[-1], 1), get_inplace_acti(args.activation)])
                else:
                    layers = ([nn.Conv1d(self.object_dim, self.hs[0], 1), get_inplace_acti(args.activation)] + 
                        sum([[nn.Conv1d(self.hs[i-1], self.hs[i], 1), get_inplace_acti(args.activation)] for i in range(1, len(self.hs) - 1)], list())
                        + [nn.Conv1d(self.hs[-2], self.hs[-1], 1), get_inplace_acti(args.activation)])
