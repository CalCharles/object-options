...
def instantiate_interaction(args, graph, environment, object_names):
    ...
    if len(args.train.dummy) != 0: # train in dummy mode
        ...
        if hasattr(interaction_model, "test") and interaction_model.test is not None and len(args.inter.interaction_testing) > 0: interaction_model.test.set_test_binaries(args.inter.interaction_testing)
