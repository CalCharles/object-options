...
class NormalizationModule():
	def __init__(self, lim_dict, dynamics_dict, object_names):
		...
		self.norm_forms = {"target": self.target_norm, "inter": self.inter_norm, "diff": self.difference_norm, "dyn": self.dynamics_norm, "rel": self.relative_norm, "raw": self.raw_norm}
		self.lim_forms = {"target": self.target_lim, "inter": self.inter_lim, "diff": self.difference_lim, "dyn": self.dynamics_lim, "rel": self.relative_lim, "raw": self.raw_lim}
	...
	def __call__(self, state, form="target", idxes=None):
		...
		if state.shape[-1] == len(norm[0]): # not multiple instanced
