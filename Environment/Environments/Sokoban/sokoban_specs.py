...
def generate_specs(rows, columns, num_obstacles, num_blocks, num_targets):
    ranges = {
    	...
    	"Pusher": [np.array([0, 0]).astype(np.float64), np.array([rows, columns]).astype(np.float64)],
    	"Obstacle": [np.array([0, 0]).astype(np.float64), np.array([rows, columns]).astype(np.float64)],
        "Block": [np.array([0, 0]).astype(np.float64), np.array([rows, columns]).astype(np.float64)],
        "Target": [np.array([0, 0, 0]).astype(np.float64), np.array([rows, columns, 1]).astype(np.float64)],
        "Bound": [np.array([0, 0]).astype(np.float64), np.array([RNG, 1]).astype(np.float64)],
    ...
    dynamics = {
        ...
        "Bound": [np.array([0, 0]).astype(np.float64), np.array([0.01, 0.01]).astype(np.float64)],
        "Done": [np.array([0]).astype(np.float64), np.array([1]).astype(np.float64)],
        "Reward": [np.array([-100]).astype(np.float64), np.array([100]).astype(np.float64)]
    }
...
# num_rows, num_columns, num_blocks, num_obstacles, num_targets, preset (load directory for human-created puzzles)
sokoban_variants = {"single": (10, 10, 1, 10, 1, 20, ""), 
