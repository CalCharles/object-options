...
def fill_buffer(environment, data, args, norm, predict_dynamics):
    ...
    for i, next_factored_state in enumerate(data[1:]):
        ...
        for name in environment.object_names:
            target = norm(args.target_select[name](factored_state))
            next_target = norm(args.target_select[name](next_factored_state))
            target_diff = norm(args.target_select[name](next_factored_state) - args.target_select[name](factored_state), form="dyn")

            # compute trace should give back if there is a true interaction at a state
            full_trace = environment.get_full_trace(factored_state, act, name)
