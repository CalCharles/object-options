...
def fill_buffer(environment, data, args, object_names, norm):
    ...
    for next_factored_state in data[1:]:
        ...
        inter = environment.get_trace(factored_state, act, object_names)
        ...
        buffer.add(Batch(act=act, done=done, rew=rew, target=target, next_target=next_target, target_diff=target_diff,
                        trace=inter, obs=inter_state, inter_state=inter_state, parent_state=parent_state, additional_state=additional_state,
                        inter=0, mask=0, weight_binary=0)) # the last row are dummy placeholders
