...
class BreakoutExtractor():
    ...
    def get_obs(self, full_state):
        ...
        block = np.concatenate([np.array(full_state['factored_state']["Block" + str(i)]) for i in range(self.num_blocks)], axis=-1)
        rel = paddle - ball
        # print(parent.shape, target.shape, rel.shape)
        if self.normalized: components = [paddle / var - 0.5, ball / var - 0.5, rel / var, block / var - 0.5]
        else: components = [paddle, ball, rel, block]
...
class RoboPushingExtractor():
    ...
    def get_obs(self, full_state):
        ...
        obstacle = np.concatenate([np.array(full_state['factored_state']["Obstacle" + str(i)]) for i in range(self.num_obstacles)], axis=-1)
        relvar = np.array([0.4,0.4, 0.05])
        rel1 = block - target
        relvar = np.array([0.4,0.4, 0.2])
        rel2 = block - target
        # print(parent.shape, target.shape, rel.shape)
        if self.normalized: components = [(gripper - gmean) / gvar, (block - bmean) / bvar, rel1 / var, rel2/var, 
        										(target - bmean) / bvar, (obstacle - bmean) / bvar]
        else: components = [gripper, block, rel1, rel2, target, obstacle]
        # print(cat, self.obs_components)
        if hasattr(self, "input_scaling"):
            return np.concatenate(components, axis=-1) * self.input_scaling
        else: return np.concatenate(components, axis=-1)
