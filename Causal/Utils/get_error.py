...
def compute_error(full_model, error_type, part, normalized = False, reduced=True):
    ...
    if error_type <= error_types.ACTIVE_RAW:
        return rv(np.stack(pytorch_model.unwrap(output)))

    # likelihood type error computation
    if error_type == error_types.LIKELIHOOD:
        output = pytorch_model.unwrap(full_model.weighted_likelihoods(part)[-1].sum(dim=-1).unsqueeze(-1))
    elif error_type == error_types.PASSIVE_LIKELIHOOD:
        output = pytorch_model.unwrap(full_model.passive_likelihoods(part)[-1].sum(dim=-1).unsqueeze(-1))
    elif error_type == error_types.ACTIVE_LIKELIHOOD:
        output = pytorch_model.unwrap(full_model.active_likelihoods(part)[-1].sum(dim=-1).unsqueeze(-1))
    if error_type <= error_types.ACTIVE_LIKELIHOOD:
        if reduced: output = np.expand_dims(np.sum(output, axis=-1), axis=1)
        return output
...
def get_error(full_model, rollouts, error_type=0, reduced=True, normalized=False):
    ...
    for i in range(int(np.ceil(len(batch) / min(500,len(batch))))): # run 500 at a time, so that we don't overload the GPU
        ...
        values = compute_error(full_model, error_type, part, normalized=normalized, reduced=reduced) * done_flags if not outputs(error_type) else compute_error(full_model, error_type, part, normalized=normalized, reduced=reduced)
