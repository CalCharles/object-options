...
def generate_counterfactual_perturbation(idx_set, full_model, batch, full_batch, args): # generates a single counterfactual perturbation
    ...
    batch.tarinter_state = np.concatenate([batch.obs, full_batch.inter_state], axis=-1)

def compute_counterfactual_cause(full_model, batch, full_batch, args):
    ...
    for i in range(full_model.num_inter):
        ...
        log_probs = list()
        for j in range(args.inter_baselines.num_counterfactuals):
            reassign_batch = generate_counterfactual_perturbation([i], full_model, batch, full_batch, args)
            ...
            log_probs.append(pytorch_model.unwrap(active_full_log_probs.sum(dim=-1).unsqueeze(-1) * done_flags).squeeze()) # don't use batch size of 1
        log_probs = np.stack(log_probs, axis=1) # shapes: batch -> batch x num_counterfactuals
        counterfactual_variance[:,i] = np.var(log_probs, axis=-1)
...
def compute_distributional_distance(actual_full, actual_log_probs, reassign_full, reassign_log_probs):
    ...
    return (actual_log_probs.sum(dim=-1).unsqueeze(-1) - reassign_log_probs.sum(dim=-1).unsqueeze(-1)).abs()

def compute_splitting(full_model, inter_masks, batch, full_batch, args, is_zero=False):
    ...
    for j in range(args.inter_baselines.num_counterfactuals):
        reassign_batch = generate_counterfactual_perturbation(np.nonzero(inter_masks), full_model, base_batch, full_batch, args)
        active_full, inter, hot_mask, full_mask, target, _, active_full_log_probs, active_full_inputs = full_model.active_open_likelihoods(reassign_batch)
