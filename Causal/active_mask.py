...
class ActiveMasking():
    def __init__(self, rollouts, interaction_model, min_sample_difference, var_cutoff, parent_active, parent_max_num, num_samples=30):
    ...
    def determine_active_set(self, rollouts, interaction_model, parent_limits, parent_active, parent_max_num):
        ...
        for i in range(len(rollouts)):
            ...
            if interaction_model.test(batch.inter): # assumes interactions are already computed
                ...
                # sample alternative states based on the 
                ...
                lin_spaces = [np.linspace(parent_limits[0][i], parent_limits[1][i], min(self.num_samples, parent_max_num)) for i in range(len(parent_limits[0])) if parent_active[i] != 0]
