...
class interaction_logger():
    def __init__(self, name, log_interval, full_model):
        self.name = name
    ...
    def log(self, i, loss, interactions, binaries, dones, trace=None, no_print=False):
        ...
        if i % self.log_interval == 0 and not no_print:
            ...
            if self.total_true > 0: log_str += f', total true: {self.total_true/self.total_seen}' # assumes that there would not be no true interactions unless unused
