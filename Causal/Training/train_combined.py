...
def train_combined(full_model, rollouts, test_rollout, args,
    ...
    inline_iter_schedule = lambda i: max(1, min(args.inter.active.inline_iters[0],
                                         args.inter.active.inline_iters[1] * np.power(2, (i/args.inter.active.inline_iters[2]))) if args.inter.active.inline_iters[2] > 0 else args.inter.active.inline_iters[0]) 
    ...
    for i in range(args.train.num_iters):
        ...
        if i % args.inter.active.active_log_interval == 0:
            ...
            interaction_weights = get_weights(inter_weighting_lambda, rollouts.weight_binary[:len(rollouts)].squeeze())
