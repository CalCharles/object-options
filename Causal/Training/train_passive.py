...
def train_passive(full_model, rollouts, args, active_optimizer, passive_optimizer, weights=None, normalize=False):
    ...
    for i in range(args.inter.passive.passive_iters):
        ...
        if normalize: target = full_model.norm(target)
        target = pytorch_model.wrap(target, cuda=full_model.iscuda)

        # compute network values
        if normalize: batch.target = full_model.norm(batch.target)
        ...
        if args.inter.passive.pretrain_active:
            if normalize: batch.inter_state = full_model.norm(batch.inter_state)
